    ...
    <main className="min-h-screen overflow-hidden bg-[#f7f3ec] text-[#1c1c1c]">
      <header className="absolute inset-x-0 top-0 z-20 border-b border-white/15 bg-[#0b3d2e]/90 text-white backdrop-blur-md">
        <div className="mx-auto flex h-[76px] max-w-[1280px] items-center justify-between px-5 lg:px-8">
          ...
          <div className="flex items-center gap-3"><a href={profileRole ? (profileRole === 'agent' ? '/dashboard/agent' : profileRole === 'property_owner' ? '/dashboard/property-owner' : profileRole === 'admin' ? '/dashboard/admin' : profileRole === 'super_admin' ? '/dashboard/super-admin' : '/dashboard/user') : '/auth/login'} className="hidden rounded-lg px-3 py-2 text-sm font-medium text-white hover:bg-white/10 sm:inline-flex">{profileRole ? t('dashboard') : t('signIn')}</a><a href={listHref} className={`inline-flex h-9 items-center gap-1.5 rounded-full px-5 text-sm font-medium ${canListProperty ? 'bg-[#c9a961] text-[#0b3d2e] hover:bg-[#e1c67e]' : 'border border-white/30 bg-white/10 text-white hover:bg-white/20'}`}>{canListProperty ? t('listProperty') : t('becomePartner')} <ArrowRight data-icon="inline-end" /></a></div>
        </div>
      </header>
