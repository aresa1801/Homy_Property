  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#f7f3ec] text-[#1c1c1c]">
      ...
      <section id="top" className="relative min-h-[590px] bg-[#0b3d2e] pt-24 text-white sm:min-h-[650px] sm:pt-32">
        ...
        <div className="absolute inset-x-0 bottom-[-112px] z-10 mx-auto max-w-[1180px] px-4 sm:bottom-[-70px] sm:px-5 lg:px-8"><div className="rounded-2xl bg-white p-2 sm:p-3 text-[#1c1c1c] shadow-2xl shadow-[#061f18]/30"><div className="flex flex-wrap items-center gap-1 border-b border-[#e8dfd3] px-2 pb-2 sm:flex-nowrap"><div className="flex gap-1"><button onClick={() => { setMode('Sale'); window.location.assign('/buy' + (location ? '?city=' + encodeURIComponent(location) : '')) }} className={'rounded-lg px-5 py-2.5 text-sm font-semibold transition ' + (mode === 'Sale' ? 'bg-[#0b3d2e] text-white' : 'text-[#65706c] hover:bg-[#f7f3ec]')}>{t('sale')}</button><button onClick={() => { setMode('Rent'); window.location.assign('/rent' + (location ? '?city=' + encodeURIComponent(location) : '')) }} className={'rounded-lg px-5 py-2.5 text-sm font-semibold transition ' + (mode === 'Rent' ? 'bg-[#0b3d2e] text-white' : 'text-[#65706c] hover:bg-[#f7f3ec]')}>{t('rent')}</button></div><span className="ml-auto hidden text-xs text-[#65706c] sm:block">{t('tellUs')}</span></div><div className="grid gap-2 p-1.5 sm:gap-3 sm:p-2 md:grid-cols-[1.4fr_1fr_1fr_auto] md:items-end"><label className="flex flex-col gap-1.5 text-xs font-semibold text-[#65706c]">{t('location')}<input value={location} onChange={(e) => setLocation(e.target.value)} placeholder={language === 'id' ? 'Kota, area, atau alamat' : 'City, neighborhood, or address'} className="h-12 rounded-lg border border-[#e8dfd3] bg-[#fcfaf7] px-4 text-sm font-normal text-[#1c1c1c] outline-none ring-[#c9a961] placeholder:text-[#9ca39e] focus:ring-2" /></label><label className="flex flex-col gap-1.5 text-xs font-semibold text-[#65706c]">{t('propertyType')}<select className="h-12 rounded-lg border border-[#e8dfd3] bg-[#fcfaf7] px-4 text-sm font-normal outline-none"><option>{t('anyType')}</option><option>{t('house')}</option><option>{t('apartment')}</option><option>{t('villa')}</option></select></label><label className="flex flex-col gap-1.5 text-xs font-semibold text-[#65706c]">{t('budget')}<select className="h-12 rounded-lg border border-[#e8dfd3] bg-[#fcfaf7] px-4 text-sm font-normal outline-none"><option>{t('anyBudget')}</option><option>Di bawah Rp 1 M</option><option>Rp 1 M — Rp 3 M</option><option>Di atas Rp 3 M</option></select></label><Button className="h-12 rounded-lg bg-[#c9a961] px-6 text-[#0b3d2e] hover:bg-[#b7964f]" onClick={() => window.location.assign((mode === 'Rent' ? '/rent' : '/buy') + (location ? '?city=' + encodeURIComponent(location) : ''))}><Search data-icon="inline-start" /> Cari</Button></div></div></div>
      </section>

      <section id="properties" className="mx-auto max-w-[1280px] px-4 pb-14 pt-48 sm:px-5 sm:pb-20 sm:pt-36 lg:px-8"><div className="mb-5 sm:mb-8 flex items-end justify-between"><div><p className="mb-3 text-sm font-semibold uppercase tracking-[.18em] text-[#c09b54]">{t('curatedForYou')}</p><h2 className="font-serif text-2xl sm:text-4xl text-[#0b3d2e] md:text-5xl">{t('propertiesYouLove')}</h2><p className="mt-3 text-[#65706c]">{t('personalizedPicks')}</p></div><div className="hidden gap-2 sm:flex"><Button variant="outline" size="icon" className="rounded-full border-[#d8ccbb]" onClick={() => setActiveSlide(Math.max(0, activeSlide - 1))}><ChevronLeft /></Button><Button variant="outline" size="icon" className="rounded-full border-[#d8ccbb]" onClick={() => setActiveSlide(Math.min(Math.max(cards.length - 1, 0), activeSlide + 1))}><ChevronRight /></Button></div></div>{/* Dua kartu per baris di HP (md ke atas tetap tiga) agar daftar rekomendasi lebih padat. */}<div className="grid grid-cols-2 gap-3 sm:gap-6 md:grid-cols-3">{cards.map((property, index) => <article key={property.id} className={`group flex flex-col overflow-hidden rounded-2xl bg-white shadow-[0_10px_35px_rgba(20,42,32,.07)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_18px_45px_rgba(20,42,32,.14)] ${index < activeSlide ? 'hidden md:block' : ''}`}><div className="relative aspect-[1.3] overflow-hidden"><img src={property.image} alt={property.title} className="size-full object-cover transition duration-500 group-hover:scale-105" /><div className="absolute left-2 top-2 rounded-full bg-[#0b3d2e] px-2 py-1 text-[10px] font-semibold text-[#f6e2a8] sm:left-4 sm:top-4 sm:px-3 sm:py-1.5 sm:text-xs">{property.tag}</div><button aria-label={`Save ${property.title}`} onClick={() => toggleFavorite(property.id)} className="absolute right-2 top-2 grid size-8 place-items-center rounded-full bg-white/90 text-[#0b3d2e] transition hover:bg-white sm:right-4 sm:top-4 sm:size-9"> <Heart className={`size-4 sm:size-5 ${favorites.includes(property.id) ? 'fill-[#a3282c] text-[#a3282c]' : ''}`} /></button></div><div className="flex flex-1 flex-col p-3 sm:p-5"><h3 className="font-serif text-sm leading-tight text-[#0b3d2e] sm:text-lg lg:text-xl">{property.title}</h3><p className="mt-1 flex items-center gap-1 text-[11px] text-[#65706c] sm:text-sm"><MapPin data-icon="inline-start" className="size-3.5 shrink-0" /><span className="truncate">{property.location}</span></p><p className="mt-2 text-sm font-bold text-[#0b3d2e] sm:mt-4 sm:text-lg">{property.price}</p><p className="mt-1 hidden text-xs text-[#8a928e] sm:block">{property.meta}</p></div></article>)}</div></section>
