...
import { BarChart3, Bell, CalendarDays, ChevronDown, ChevronRight, FileSignature, Flag, Home, LayoutDashboard, LogOut, Menu, Search, Settings, ShieldCheck, Sparkles, Users, WalletCards, X } from 'lucide-react'
...
export function DashboardShell({ role, children, showSummary = true }: { role: Role; children: React.ReactNode; showSummary?: boolean }) {
  ...
  return <div className="min-h-screen bg-[#f7f3ec] text-[#20332c]">
    <aside className={`fixed inset-y-0 left-0 z-30 flex w-72 flex-col border-r border-[#e5dccd] bg-[#0b3d2e] p-6 text-white transition-transform lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <div className="mt-3 shrink-0 rounded-xl border border-white/10 bg-white/5 p-4"><p className="text-xs uppercase tracking-[.18em] text-[#c9a961]">Ruang Kerja</p><p className="mt-2 font-semibold">Dasbor {translated(role)}</p><p className="mt-1 text-xs text-white/55">Kelola aktivitas Homy Anda</p></div>
      <nav className="mt-6 flex min-h-0 flex-1 flex-col gap-1 overflow-y-auto pr-1">{links.map(({ label, href, icon: Icon }) => { const active = href === activeHref; return <Link key={label} href={href} aria-current={active ? 'page' : undefined} onClick={() => setOpen(false)} className={`group flex shrink-0 items-center gap-3 rounded-xl px-4 py-2.5 text-sm transition ${active ? 'bg-[#c9a961] font-semibold text-[#0b3d2e] shadow-[0_8px_20px_rgba(201,169,97,.22)]' : 'text-white/70 hover:bg-white/10 hover:text-white'}`}><Icon className={`size-[18px] shrink-0 ${active ? 'text-[#0b3d2e]' : 'text-white/55 group-hover:text-white'}`} /><span className="truncate">{translated(label)}</span></Link> })}</nav>
