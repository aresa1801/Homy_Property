...
export function UserDashboardPage({ type }: { type: PageType }) {
  ...
  return <DashboardShell role="User"><div className="mb-5 sm:mb-8"><p className="text-sm font-semibold uppercase tracking-[.16em] text-[#a18a61]">Dasbor Pengguna</p><h2 className="mt-2 font-serif text-2xl sm:text-4xl text-[#0b3d2e]">{current.title}</h2><p className="mt-3 max-w-2xl text-[#718078]">{current.intro}</p></div><SectionCard title={current.title}><div className="space-y-3">{isLoading ? <p className="text-sm text-[#718078]">Memuat data...</p> : error ? <p role="alert" className="text-sm text-red-700">Data belum dapat dimuat. Silakan coba lagi.</p> : type === 'apply' ? <div className="grid gap-4 md:grid-cols-2"><a href="/verify?role=agent" className="group rounded-2xl border border-[#e5dccd] p-4 sm:p-5 transition hover:-translate-y-0.5 hover:border-[#c9a961] hover:bg-[#f7f3ec]"><h3 className="font-semibold text-[#0b3d2e]">Menjadi Agen Properti</h3><p className="mt-2 text-sm text-[#718078]">Mulai verifikasi mitra: isi data diri, unggah KTP/SIM, atur ketersediaan waktu, lalu tanda tangani Perjanjian Kerja Sama.</p><span className="mt-4 inline-block text-sm font-semibold text-[#9a783c]">Mulai verifikasi agen →</span></a><a href="/verify?role=property_owner" className="group rounded-2xl border border-[#e5dccd] p-4 sm:p-5 transition hover:-translate-y-0.5 hover:border-[#c9a961] hover:bg-[#f7f3ec]"><h3 className="font-semibold text-[#0b3d2e]">Menjadi Pemilik Properti</h3><p className="mt-2 text-sm text-[#718078]">Mulai verifikasi mitra: lengkapi data diri dan dokumen, lalu tanda tangani Perjanjian Kerja Sama untuk memasang properti.</p><span className="mt-4 inline-block text-sm font-semibold text-[#9a783c]">Mulai verifikasi pemilik →</span></a></div> : rows.length ? rows.map((row: Record<string, unknown>, index: number) => {
    if (type === 'favorites') {
