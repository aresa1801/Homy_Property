...
export function DashboardDataSummary({ role }: { role: DashboardRole }) {
  ...
  return <div className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{config.keys.map((key, index) => <div key={key} className="rounded-2xl border border-[#e5dccd] bg-white p-5 shadow-[0_10px_30px_rgba(20,42,32,.04)]"><p className="text-sm text-[#718078]">{config.names[index]}</p><p className="mt-3 font-serif text-3xl text-[#0b3d2e]">{String(data?.metrics?.[key] ?? 0)}</p><p className="mt-2 text-xs font-semibold text-[#4e866d]">Tersinkron dengan database</p></div>)}</div>
}
