...
export function buildAgreementPdf(input: AgreementPdfInput): Uint8Array {
  ...
  for (const clause of clauses) {
    blocks.push({ type: 'heading', text: clause.title })
    for (const paragraph of clause.paragraphs ?? []) blocks.push({ type: 'paragraph', text: paragraph })
    if (clause.items?.length) blocks.push({ type: 'bullets', items: clause.items })
  }

  blocks.push({ type: 'divider' })
  blocks.push({ type: 'heading', text: 'Penandatanganan' })
  blocks.push({
    type: 'paragraph',
    text:
      'Perjanjian Kerja Sama ini ditandatangani secara elektronik oleh para pihak. Dengan tanda tangan digital ini, MITRA menyatakan telah membaca, memahami, dan menyetujui seluruh isi Perjanjian beserta lampirannya.',
  })
  blocks.push({
    type: 'signature',
    columns: [
      {
        ...
        role: `Ditandatangani digital pada ${formatDateId(input.signedAt)}`,
        meta: [
          `${identityLabel(partner.identityType)}: ${String(partner.identityNumber ?? '-')}`,
          `Email: ${String(partner.email ?? '-')}`,
          `Verifikasi: ${String(input.verificationId ?? '-')}`,
        ],
      },
      {
        label: 'PIHAK PERTAMA — HOMY',
        name: 'Admin Kemitraan Homy',
        role: COMPANY_LEGAL,
        meta: [`${COMPANY_SITE}`, COMPANY_EMAIL, `Perjanjian ${number}`],
      },
    ],
  })
